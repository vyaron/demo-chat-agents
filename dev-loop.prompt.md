...
* **Strict File Boundaries**: The code-first script uses filesystem hooks (`AGENT_ROLE`) to physically block the Frontend agent from touching Backend files. The `task` sub-agents rely on soft boundaries (system prompts) to adhere to these rules unless strictly configured via permissions.
* **Cost Tracking**: The node script tracks precise JSON token usage per CLI invocation. As an embedded Copilot agent, you do not have direct access to your own token billing metrics to write to `.orchestrate/trace.json`.
...
- Read `.doc/product-definition.md` to understand the feature requirements.
...
- Write the ticket URLs and IDs to `.orchestrate/tickets.json`.
...
- **Prompt**: "You are the Frontend Agent. Read `AGENTS.md` for global rules and `.claude/agents/frontend.md` for your specific role. Follow them exactly. Implement the frontend scope for the task. Use the Figma details from the plan. Define the API contract in `.orchestrate/api-contract.yaml`. Run `npm test` and `npx playwright test`. End your final response with STATUS: DONE."
- Monitor it using the `read_agent` tool.
- Once done, write its summary to `.orchestrate/frontend-agent-report.md`. Move the FE Linear ticket to "Done" and BE ticket to "In Progress".

### 6. Backend Agent
- Use the `task` tool to launch a background `general-purpose` agent.
- **Prompt**: "You are the Backend Agent. Read `AGENTS.md` for global rules and `.claude/agents/backend.md` for your specific role. Follow them exactly. Implement the backend scope reading `.orchestrate/api-contract.yaml` as your absolute truth. Run backend tests. End your final response with STATUS: DONE."
- Monitor it using the `read_agent` tool.
- Once done, write its summary to `.orchestrate/backend-agent-report.md`. Move the BE Linear ticket to "Done" and QA ticket to "In Progress".

### 7. QA Agent
- Use the `task` tool to launch a background `general-purpose` agent.
- **Prompt**: "You are the QA Agent. Read `AGENTS.md` for global rules and `.claude/agents/qa.md` for your specific role. Follow them exactly. Validate the feature across frontend, backend, and e2e. Write your validation results to `.orchestrate/qa-report.md` and end with STATUS: DONE."
